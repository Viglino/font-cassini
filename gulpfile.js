...
var fontName = 'font-cassini';    // set name of your symbol font
const className = 'fc'            // set class name in your CSS
...
/** Get glyphs */
var font = JSON.parse(fs.readFileSync('./font-cassini.json'));
...
function mapGlyphs (glyph) {
  ...
  if (!resp.order) resp.order = Math.round(((new Date()) - (new Date('2024')))/1000);
...
gulp.task('store', function(){
  return gulp.src(['svg/**/u*.svg'])
    .pipe(rename(function (path) {
      path.dirname = '';
      path.basename = 'fc-' + path.basename.replace(/u([^-]*)-/,'');
    }))
    .pipe(svgmin({
      plugins: [{
