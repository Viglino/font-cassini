...
function setUrl(search, select) {
  ...
  if (typeof(select) === 'string') {
    if (select) opt.push('fc='+select.replace(/^fc-/,''));
  } else {
    if (data.fc) opt.push('fc='+data.fc);
  }
  opt = opt.join('&');
  if (opt) url = url + '?' + opt;
...
function showIcon(icon, url) {
  if (icon && icon.code) {
    currentGlyph = icon;
    $('#icon i').removeClass().addClass('fc-'+icon.name)
    $('#icon h2').text('fc-' + icon.name);
...
$.ajax({
  ...
  success: function(font) {
    ...
    search(data.q);
    if (data.fc) {
      showIcon(glyphs['fc-'+data.fc]);
      showPage('icons');
    } else {
      showIcon(glyphs['fc-earth'], false);
    }
  }
});